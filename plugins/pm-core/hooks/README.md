...
Registered automatically by `hooks/hooks.json` when the `pm-core` plugin is enabled — no manual `settings.json` edit needed:
  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "sh ${CLAUDE_PLUGIN_ROOT}/hooks/scripts/bash-worktree-fix.sh",
          "timeout": 5
        }
      ]
    }
...
scripts/bash-worktree-fix.sh "ls -la"

# Test in worktree (should inject cd)
cd /path/to/worktree
scripts/bash-worktree-fix.sh "npm install"
...
1. **Verify the hook is executable:**
   ```bash
   chmod +x scripts/bash-worktree-fix.sh
   ```
...
3. **Test the hook manually with a sample command:**
   ```bash
   cd /path/to/worktree
   scripts/bash-worktree-fix.sh "npm test"
   ```

4. **Check that `hooks/hooks.json` is valid JSON:**
   ```bash
   cat hooks/hooks.json | python -m json.tool
   ```

> **Note:** this script reads the command from `argv`/`"$*"` and prints the (possibly rewritten) command line to stdout. Claude Code's `PreToolUse` hook protocol passes the full tool-call payload as JSON on stdin and expects either an exit code or JSON on stdout to signal the decision. Confirm the two line up in your Claude Code version before relying on this in production — the wiring in `hooks.json` is structurally correct, but this script predates that protocol and may need a small JSON-in/JSON-out adapter.
