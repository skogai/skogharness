...
CCPM ships no application — it is a Claude Code **plugin marketplace**: four
...
**`.claude/skills/run-ccpm/driver.sh`**.
...
```bash
bash .claude/skills/run-ccpm/driver.sh lint      # static checks on plugins/, no sandbox
...
Claude Code would set for the installed plugin and run the repo copy directly:
...
commands visible under `.claude/commands/`). Plugins resolve their own paths
via `${CLAUDE_PLUGIN_ROOT}`, so the user's project holds runtime data only:

```bash
mkdir -p .claude/prds .claude/epics    # exactly what init.sh creates
```
...
`find .claude/epics/*/updates/ -name "*.md"`. In a repo with no epics the glob
...
greps all of `.claude/`, and that includes `SKILL.md` and `driver.sh`. Writing
...
Claude Code matches a skill's `description` against the task. That makes the
