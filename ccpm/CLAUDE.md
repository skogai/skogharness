...
CCPM (Claude Code PM) is not an application — it is a **bundle of Claude Code assets** (slash commands, subagent definitions, behavioral rules, and bash helper scripts) that users install into _their_ project. There is no source code, no build step, no package manager, and no test suite. Everything here is Markdown or POSIX shell.

The "program" is the Markdown: command files in `.claude/commands/` are prompts that Claude executes, and rule files in `.claude/rules/` are shared specs those prompts include by reference. Editing a command means editing prose that another Claude instance will follow literally, so precision matters more than style.

## Layout

```
.claude/commands/{pm,context,testing}/  # slash commands (/pm:*, /context:*, /testing:*)
.claude/agents/                         # subagent definitions (context firewalls)
.claude/rules/                          # shared behavioral specs referenced by commands
.claude/scripts/pm/                     # bash backing the read-only reporting commands
.claude/hooks/                          # bash-worktree-fix.sh pre-tool-use hook
.claude/{prds,epics}/                   # runtime workspace placeholders
...
## Critical: the `.claude/` vs `.claude/` split

Commit `e0fc1a8` renamed the asset directory from `.claude/` to `.claude/`, but **only the command frontmatter and `!bash` invocation lines were updated**. Everything else still says `.claude/`:

- **Updated to `.claude/`** — the ~14 script-wrapper commands (`allowed-tools: Bash(bash .claude/scripts/pm/status.sh)` and the matching `!bash .claude/scripts/pm/status.sh`).
- **Still `.claude/`** — all script bodies, all instruction prose in command/rule/agent files, `settings.json.example`, `hooks/README.md`, and the installer docs. Roughly 229 `.claude/` references vs 28 `.claude/` ones.

Some of this is intentional and some is stale, and the two cases look identical, so decide by role:

- **Shipped assets** (commands, rules, agents, scripts, hooks) live under `.claude/` in this repo. A prose reference to `.claude/commands/...` or `.claude/rules/...` is stale.
- **Runtime data** (`prds/`, `epics/`, `context/`) is created under `.claude/` in the _user's_ project — `scripts/pm/init.sh` does `mkdir -p .claude/prds .claude/epics` and every reporting script reads from `.claude/epics`. Those references are correct; do not "fix" them to `.claude/`.

When adding a new command, follow the current convention: `.claude/` for the script path in frontmatter/`!bash`, `.claude/` for PRD/epic/task data paths in the prose.

## Two kinds of commands

1. **Script-backed** (`status`, `next`, `blocked`, `standup`, `epic-list`, `validate`, …) — the `.md` file is four lines: frontmatter declaring the exact `Bash(...)` invocation, then `!bash .claude/scripts/pm/<name>.sh $ARGUMENTS`. All logic lives in the `.sh`. Use this for deterministic, read-only reporting.
...
Read the relevant file in `.claude/rules/` before touching a command that does the corresponding thing — the commands themselves point at them, and consistency across ~40 command files is the whole point.
...
```bash
bash -n .claude/scripts/pm/<changed>.sh        # syntax check every touched script
bash .claude/scripts/check-path-standards.sh   # absolute-path / privacy scan, must exit 0
bash .claude/scripts/fix-path-standards.sh     # auto-fixes violations (writes .backup files)
```
