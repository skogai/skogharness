...
- **Runtime data** (`prds/`, `epics/`, `context/`) is created under `.claude/` in the _user's_ project — `scripts/pm/init.sh` does `mkdir -p .claude/prds .claude/epics` and every reporting script reads from `.claude/epics`. Those references are correct; do not "fix" them to `ccpm/`.
